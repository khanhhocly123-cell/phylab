      ...
      {showParts.data && (
        <section style={cardStyle}>
          ...
          <div style={{ display: "flex", alignItems: "baseline", gap: 6, margin: "2px 0 8px" }}>
            ...
            {justRolled && <span style={{ fontSize: 11, color: C.sub, marginLeft: "auto" }}>g ≈ <b style={{ color: C.ink }}>{gFromMeasurement(s, parseFloat(led)).toFixed(2)}</b></span>}
          </div>
...
function FallScene(props) {
  ...
  const viewBoxStr = 
    ...
    (props.isMobile ? "180 20 640 440" : `0 0 ${VBW} ${VBH}`);
