...
export default function LabBench({ measuredD = 20.0, studentName, assignedSets, assistantSettings, onExportNote, onBack, onReplayPrelab, speak, muted, onToggleMute }) {
  ...
  const isTeacherAssigned = Boolean(assignedSets?.[lab]?.length);
  ...
  const renderAssignmentCard = () => (
    <section data-lab-assignment style={{ ...cardStyle, background: "#FFF8F1", border: `1.5px solid ${C.orange}55` }}>
      ...
      <div style={{ fontSize: isMobile ? 11 : 12, color: C.sub, lineHeight: 1.4, marginBottom: 8 }}>
        Đo đúng từng cấu hình dưới đây; mỗi dòng là một yêu cầu riêng.
      </div>
      <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
        {currentTargets.map((target, index) => {
          const used = isTargetMeasured(target);
          return (
            <div key={`${target.theta}-${target.sEF ?? index}`} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, padding: "7px 9px", borderRadius: 8, border: `1px solid ${used ? C.good : C.line}`, background: used ? "#F3F8F3" : "#fff", fontSize: isMobile ? 11.5 : 12.5, color: C.ink }}>
              <b>Câu {index + 1}: θ={target.theta}°{lab === "average" ? ` · sEF=${(target.sEF * 100).toFixed(0)}cm` : ""}</b>
  ...
  return (
    <div className="phy-screen" data-lab-engine="inclined" style={{ flex: 1, minHeight: 0, overflow: "hidden", background: C.bg, fontFamily: FONT, display: "flex", flexDirection: "column" }}>
      <div data-lab-header style={{
        ...
        <div style={{ gridColumn: isPortrait ? "1 / -1" : "2", gridRow: isPortrait ? "2" : "1", display: "flex", justifyContent: "center", width: isPortrait ? "100%" : "auto", minWidth: 0 }}>
          <div role="tablist" aria-label="Chọn phép đo vận tốc" style={{ display: "flex", gap: 4, background: C.peachLt, padding: 3, borderRadius: 10, border: `1px solid ${C.line}`, width: isPortrait ? "100%" : "auto", maxWidth: "100%" }}>
            {[["average", "Vận tốc trung bình"], ["instant", "Vận tốc tức thời"]].map(([k, t]) => (
              <button key={k} role="tab" aria-selected={lab === k} onClick={() => setLab(k)} style={{ ...tabBtn, flex: isMobile ? 1 : "initial", whiteSpace: "nowrap", fontSize: isMobile ? 11 : 12.5, padding: isMobile ? "4px 8px" : "6px 14px", ...(lab === k ? tabActive : {}) }}>{isMobile && k === "average" ? "Vận tốc TB" : t}</button>
            ))}
          </div>
        </div>

        <div data-lab-meta style={{ gridColumn: "3", gridRow: "1", justifySelf: "end", fontSize: isMobile ? 11 : 12, color: C.sub, whiteSpace: "nowrap" }}>d = <b style={{ color: C.ink }}>{measuredD.toFixed(2)} mm</b></div>
      </div>
      ...
      <div data-lab-layout data-orientation={isPortrait ? "portrait" : "landscape"} style={isMobile
        ...
        {/* MOBILE BOTTOM SHEET / DESKTOP SIDEBAR: Trợ lý / Tiến trình / Ghi số liệu */}
        {isMobile ? (
